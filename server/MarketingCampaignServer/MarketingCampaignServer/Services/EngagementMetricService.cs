...
namespace MarketingCampaignServer.Services
{
    public class EngagementMetricService : IEngagementMetricService
    {
        ...
        // Update metric: prefer counts when provided, else accept direct rates
        public async Task<EngagementMetricDto?> UpdateMetricAsync(UpdateEngagementMetricDto dto, long modifiedByUserId)
        {
            ...
            if (countsProvided)
            {
                ...
                // If you later add raw count columns to entity, set them here
                // metric.EmailsSent = emailsSent; ...
            }
            else
            {
                // allow direct rate updates (optional)
        ...
        // Interface-required: allow external trigger to recalc (wraps internal helper)
        ...
        // Private helper that recalculates campaign-level OpenRate & ConversionRate
